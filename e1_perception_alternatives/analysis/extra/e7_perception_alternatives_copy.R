...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/serial_self/e7_perception_alternatives/data")
...
#one self #2
d_oneAlt <- subset(d.mat, d.mat$mainCond==3)

mean(d_oneAlt$d[d_oneAlt$agentCond==1]) #future-you2
sd(d_onAlte$d[d_oneAlt$agentCond==1])
...
tes(as.numeric(att_1_o[1alt), n_o_1, n_o_2) #cohen's d
tes(as.numeric(att_2_o[1alt[1]), n_o_1, n_o_3) #cohen's d
tes(as.numeric(att_3_alt), n_o_2, n_o_3) #cohen's d
...
p1.111<-p1.11+scale_fill_discrete(name = "", labels = c ("Future-You1", "Stranger-John", "Stranger-Bill")) +
  xlab ("") + ylab ("") +
  ...
  geom_signif(data=d.one,
              aes(xmin=1, xmax=2, annotations=star_mat[1], y_position=3.4),
...
p1.114<-p1.14+scale_fill_discrete(name = "", labels = c ("Future-You1", "Future-You2", "Stranger-John")) +
  ...
  geom_signif(data=d.two,
              aes(xmin=1, xmax=2, annotations=star_mat[4], y_position=2.7),
...
, vjust = -1.0annotate_figure(figure,top = text_grob("Experiment 8",7: Alternate Future Selvesface = "bold", size = 14)27                left = text_grob("d' Performance", color="black", face ="plain",size=27, rot=90)) 
