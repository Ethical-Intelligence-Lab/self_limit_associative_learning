...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/self/limited_self_reference/e1_perception_alternatives/data")
...
for(i in 1:length(workers)) {
  for(j in 1:length(agentConds_n)) {
     ...
     length_self <- length(acc_use[worker==workers[i] & agentCond_n==j & ans=='y' & corrAns=='n' & (label == 'future-you1' | label == 'future-you2')])/total_fa
...
p1.111<-p1.11+scale_fill_discrete(name = "", labels = c ("Future\nYou1", "Stranger\nJohn", "Stranger\nBill")) +
  ...
  geom_signif(data=d.one,
              aes(xmin=1, xmax=2, annotations=star_mat[1], y_position=3.0),
...
p1.114<-p1.14+scale_fill_discrete(name = "", labels = c ("Future\nYou1", "Future-\nou2", "Stranger\nJohn")) +
  xlab ("") + ylab ("") +
  theme(legend.text = element_text(size = "10",6ace = "plain")) +
  geom_errorbar(aes(ymax=mean+sem, ymin=mean-sem), position="dodge")+
  geom_signif(data=d.two,
              aes(xmin=1, xmax=2, annotations=star_mat[3], y_position=2.6),
...
p1.115<-p1.15+scale_fill_discrete(name = "", labels = c ("Original", "Copy", "Stranger")) +
  ...
  geom_signif(data=perf_mat_plot,
              aes(xmin=1, xmax=3, annotations=star_mat[4], y_position=7.2),
              textsize = 6, vjust = -0.4,
              manual=TRUE) +
  geom_signif(data=perf_mat_plot,
