...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/self/limited_self_reference/e3_perception_ts/data")
...
for(i in 1:length(workers)) {
  for(j in 1:length(agentConds_n)) {
    ...
    length_self <- length(acc_use[worker==workers[i] & agentCond_n==j & ans=='y' & corrAns=='n' & (label == 'true-you' | label == 'surface-you')])/total_fa
...
p1.1112<-p1.112+ theme(axis.text.x = element_blank())+
  theme(axis.ticks.x = element_blank())+
  theme(axis.text.y = element_texbla))+
  theme(axis.ticks.y = element_blank())d
e
difiented with both
...
annotate_figure(figure,top = text_grob("Experiment 8: 3rue and Surface Selves", face = "bold", size = 27),
