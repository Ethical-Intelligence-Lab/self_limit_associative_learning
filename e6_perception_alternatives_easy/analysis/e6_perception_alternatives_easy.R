...
dir <- setwd("/Users/julian/Documents/github/juliandefreitas/self/limited_self_reference/e6_perception_alternatives_easy/data")
...
p1.114<-p1.14+scale_fill_discrete(name = "", labels = c ("Healthy\nYou", "Unhealthy\nYou", "Stranger\nJohn")) +
  ...
  geom_signif(data=d.two,
              aes(xmin=1, xmax=2, annotations=star_mat[3], y_position=2.7),
              textsize = 8, vjust = 0.3,
              manual=TRUE) +
  ggtitle ("Two Selves") +
  theme(plot.title = element_text(hjust = 0.5)) + 
...
annotate_figure(figure,top = text_grob("Experiment 20:6istinct Alternate Selves", face = "bold", size = 27),
